...
impl Heap {
    ...
    pub fn realloc(&mut self, memory: &mut Memory, offset: usize, new_size: usize) -> Result<usize, HeapError> {
        ...
        memory.perms_mut_raw(memory.heap(), self.cursor - memory.heap()).copy_within(offset..offset + copy_len, new_chunk);
        memory.content_mut_raw(memory.heap(), self.cursor - memory.heap()).copy_within(offset..offset + copy_len, new_chunk);
