...
impl Runtime for MultiverseRuntime {
    ...
    fn run(&mut self) -> Result<Self::Event, Self::Error> {
        ...
        /* Execute code */
        self.next_pc = self.executor.run(&mut self.globals, &mut self.heap, &mut self.stack, &mut self.event_channel, &mut self.registers, &mut self.var_storage);
    ...
    fn take_snapshot(&mut self, id: SnapshotId) {
        ...
        self.globals.take_snapshot(id);
        self.heap.take_snapshot(id);
        self.stack.take_snapshot(id);
    ...
    fn restore_snapshot(&mut self, id: SnapshotId) -> Result<(), Self::Error> {
        ...
        self.globals.restore_snapshot_unchecked(id);
        self.heap.restore_snapshot_unchecked(id);
        self.stack.restore_snapshot_unchecked(id);
    ...
    fn delete_snapshot(&mut self, id: SnapshotId) -> Result<(), Self::Error> {
        ...
        self.globals.delete_snapshot_unchecked(id);
        self.heap.delete_snapshot_unchecked(id);
        self.stack.delete_snapshot_unchecked(id);
    ...
    fn load_dword(&self, address: VAddr) -> Result<u64, Self::Error> {
        let result = match AddressSpace::decode(address) {
            ...
            AddressSpace::Global(offset) => load_riscv_type(&self.globals, offset),
            AddressSpace::Heap(offset) => load_riscv_type(&self.heap, offset),
            AddressSpace::Stack(offset) => load_riscv_type(&self.stack, offset),
        };
    ...
    fn load_slice(&self, address: VAddr, size: usize) -> Result<&[u8], Self::Error> {
        let result = match AddressSpace::decode(address) {
            AddressSpace::Code(_) => None,
            AddressSpace::Global(offset) => load_slice(&self.globals, offset, size),
            AddressSpace::Heap(offset) => load_slice(&self.heap, offset, size),
            AddressSpace::Stack(offset) => load_slice(&self.stack, offset, size),
        };
    ...
    fn store_dword(&mut self, address: VAddr, value: u64) -> Result<(), Self::Error> {
        let result = match AddressSpace::decode(address) {
            ...
            AddressSpace::Global(offset) => store_riscv_type(&mut self.globals, offset, value),
            AddressSpace::Heap(offset) => store_riscv_type(&mut self.heap, offset, value),
            AddressSpace::Stack(offset) => store_riscv_type(&mut self.stack, offset, value),
        };
        if result {
            Ok(())
        } else {
    ...
    fn store_slice<S: AsRef<[u8]>>(&mut self, address: VAddr, value: S) -> Result<(), Self::Error> {
        ...
        let result = match AddressSpace::decode(address) {
            AddressSpace::Code(_) => false,
            AddressSpace::Global(offset) => store_slice(&mut self.globals, offset, value),
            AddressSpace::Heap(offset) => store_slice(&mut self.heap, offset, value),
            AddressSpace::Stack(offset) => store_slice(&mut self.stack, offset, value),
        };
        if result {
            Ok(())
        } else {
    ...
    fn load_string(&self, address: VAddr) -> Result<&[u8], Self::Error> {
        let result = match AddressSpace::decode(address) {
            AddressSpace::Code(_) => None,
            AddressSpace::Global(offset) => load_string(&self.globals, offset),
            AddressSpace::Heap(offset) => load_string(&self.heap, offset),
            AddressSpace::Stack(offset) => load_string(&self.stack, offset),
        };
    ...
    fn store_string<S: AsRef<str>>(&mut self, address: VAddr, value: S) -> Result<(), Self::Error> {
        ...
        let result = match AddressSpace::decode(address) {
            AddressSpace::Code(_) => false,
            AddressSpace::Global(offset) => store_string(&mut self.globals, offset, value),
            AddressSpace::Heap(offset) => store_string(&mut self.heap, offset, value),
            AddressSpace::Stack(offset) => store_string(&mut self.stack, offset, value),
        };
        if result {
            Ok(())
        } else {
...
impl MultiverseRuntime {
    ...
    pub fn permissions(&self, address: VAddr, size: usize) -> Result<&[u8], MultiverseRuntimeFault> {
        let result = match AddressSpace::decode(address) {
            AddressSpace::Code(_) => None,
            AddressSpace::Global(offset) => load_perms(&self.globals, offset, size),
            AddressSpace::Heap(offset) => load_perms(&self.heap, offset, size),
            AddressSpace::Stack(offset) => load_perms(&self.stack, offset, size),
        };
        result.ok_or(MultiverseRuntimeFault::MemoryReadError(address, size))
    }
    ...
    pub fn permissions_mut(&mut self, address: VAddr, size: usize) -> Result<&mut [u8], MultiverseRuntimeFault> {
        let result = match AddressSpace::decode(address) {
            AddressSpace::Code(_) => None,
            AddressSpace::Global(offset) => load_perms_mut(&mut self.globals, offset, size),
            AddressSpace::Heap(offset) => load_perms_mut(&mut self.heap, offset, size),
            AddressSpace::Stack(offset) => load_perms_mut(&mut self.stack, offset, size),
        };
    ...
    pub fn load_slice_mut(&mut self, address: VAddr, size: usize) -> Result<&mut [u8], MultiverseRuntimeFault> {
        let result = match AddressSpace::decode(address) {
            AddressSpace::Code(_) => None,
            AddressSpace::Global(offset) => load_slice_mut(&mut self.globals, offset, size),
            AddressSpace::Heap(offset) => load_slice_mut(&mut self.heap, offset, size),
            AddressSpace::Stack(offset) => load_slice_mut(&mut self.stack, offset, size),
        };
        result.ok_or(MultiverseRuntimeFault::MemoryReadError(address, size))
    }
...
impl MultiverseRuntime {
    pub fn dynstore_allocate(&mut self, size: usize) -> Result<VAddr, MultiverseRuntimeFault> {
        let offset = self.heap_mgr.malloc(&mut self.heap, size)?;
        Ok(AddressSpace::Heap(offset).encode())
    }
    ...
    pub fn dynstore_get_single_chunk(&self, addr: VAddr) -> Result<HeapChunk, MultiverseRuntimeFault> {
        ...
        let chunk = self.heap_mgr.get_heap_chunk(&self.heap, offset)?;
    ...
    pub fn dynstore_reallocate(&mut self, addr: VAddr, new_size: usize) -> Result<VAddr, MultiverseRuntimeFault> {
        ...
        let new_offset = self.heap_mgr.realloc(&mut self.heap, offset, new_size)?;
        Ok(AddressSpace::Heap(new_offset).encode())
    }
    ...
    pub fn dynstore_memory_usage(&self) -> usize {
        self.heap_mgr.memory_usage()
    }
}

#[cfg(test)]
impl MultiverseRuntime {
    pub(crate) fn stack(&self) -> &Memory {
        &self.stack
    }
}
