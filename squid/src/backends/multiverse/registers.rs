...
/// [32x GP] [32x FP] [fcsr] [pc] [instr]
const REGISTER_COUNT: usize = 32 + 32 + 1 + 1 + 1;

pub(crate) const INSTR_INDEX: usize = 66;
pub(crate) const PC_INDEX: usize = 65;
...
impl Registers {
    ...
    pub(crate) fn get_fp(&self, reg: usize) -> u64 {
        *unsafe { self.content.get_unchecked(32 + reg) }
    }

    pub(crate) fn set_fp(&mut self, reg: usize, value: u64) {
        *unsafe { self.content.get_unchecked_mut(32 + reg) } = value;
    }

    pub(crate) fn get_csr(&self, csr: CsrRegister) -> u64 {
        let value = *unsafe { self.content.get_unchecked(64) };
        match csr {
    ...
    pub(crate) fn set_csr(&mut self, csr: CsrRegister, value: u64) {
        let old_value = *unsafe { self.content.get_unchecked(64) };
