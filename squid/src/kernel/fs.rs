...
mod tests {
    ...
    fn test_mkdir_1() {
        let mut fs = Fs::new();
        let root = fs.get_directory_handle("/").unwrap();
        fs.mkdir(root, "asdf", PERM_W | PERM_R).unwrap();
        let asdf = fs.get_directory_handle("/asdf/").unwrap();
        fs.mkdir(asdf, "abc", PERM_W | PERM_R).unwrap();
        fs.get_directory_handle("/asdf/abc/").unwrap();
    }
    ...
    fn test_handles() {
        let mut fs = Fs::new();
        let root = fs.get_directory_handle("/").unwrap();
        fs.mkdir(root, "folder", PERM_W | PERM_R).unwrap();
        fs.touch(root, "file", PERM_W | PERM_R).unwrap();
        fs.get_file_handle("/file").unwrap();
        assert!(fs.get_directory_handle("/file").is_err());
        assert!(fs.get_file_handle("/folder").is_err());
    }
    ...
    fn test_files() {
        let mut fs = Fs::new();
        let root = fs.get_directory_handle("/").unwrap();
        fs.touch(root, "file", 0).unwrap();
        let handle = fs.get_file_handle("/file").unwrap();
        let file = fs.file(handle).unwrap();
        println!("{:?}", file);
    }
    ...
    fn bench_handle() {
        ...
        fs.mkdir(root, "a", PERM_R | PERM_W).unwrap();
        let a = fs.get_directory_handle("/a/").unwrap();

        fs.mkdir(a, "b", PERM_R | PERM_W).unwrap();
        let b = fs.get_directory_handle("/a/b/").unwrap();

        fs.mkdir(b, "c", PERM_R | PERM_W).unwrap();
        let c = fs.get_directory_handle("/a/b/c/").unwrap();

        fs.touch(c, "d", 0).unwrap();

        let trials = 10_000_000;
        let start = std::time::Instant::now();

        for _ in 0..trials {
            let file = fs.get_file_handle("/a/b/c/d").unwrap();
            fs.file(file).unwrap();
        }

        let duration = std::time::Instant::now() - start;

        println!("trials/s: {:.02}", (trials as f64) / duration.as_secs_f64());
    }
    ...
    fn test_snapshot_1() {
        ...
        let d = fs.get_file_handle("/a/b/c/d").unwrap();
        fs.rm(d).unwrap();

        fs.take_snapshot(3);

        /* test snapshot 0 */
        assert!(fs.restore_snapshot(0));
        assert!(fs.directory(a).is_err());
        assert!(fs.directory(b).is_err());
        assert!(fs.directory(c).is_err());
        assert!(fs.file(d).is_err());
        assert!(fs.get_directory_handle("/a/").is_err());

        /* test snapshot 1 */
        assert!(fs.restore_snapshot(1));
        assert!(fs.directory(a).is_ok());
        assert!(fs.directory(b).is_ok());
        assert!(fs.directory(c).is_err());
        assert!(fs.file(d).is_err());
        assert!(fs.get_directory_handle("/a/b/").is_ok());
        assert!(fs.get_directory_handle("/a/b/c/").is_err());

        /* test snapshot 2 */
        assert!(fs.restore_snapshot(2));
        assert!(fs.directory(a).is_ok());
        assert!(fs.directory(b).is_ok());
        assert!(fs.directory(c).is_ok());
        assert!(fs.file(d).is_ok());
        assert!(fs.get_directory_handle("/a/b/").is_ok());
        assert!(fs.get_directory_handle("/a/b/c/").is_ok());
        assert!(fs.get_file_handle("/a/b/c/d").is_ok());

        /* test snapshot 3 */
        assert!(fs.restore_snapshot(3));
        assert!(fs.directory(a).is_ok());
        assert!(fs.directory(b).is_ok());
        assert!(fs.directory(c).is_ok());
        assert!(fs.file(d).is_err());
        assert!(fs.get_directory_handle("/a/b/").is_ok());
        assert!(fs.get_directory_handle("/a/b/c/").is_ok());
        assert!(fs.get_file_handle("/a/b/c/d").is_err());
    }
    ...
    fn test_snapshot_2() {
        ...
        fs.touch(root, "d", 0).unwrap();
        let d = fs.get_file_handle("/d").unwrap();
    ...
    fn test_snapshot_3() {
        ...
        fs.mkdir(root, "a", 0).unwrap();
        let a_dir = fs.get_directory_handle("/a/").unwrap();
        fs.take_snapshot(1);

        assert!(fs.restore_snapshot(0));

        fs.touch(root, "a", 0).unwrap();
        let a_file = fs.get_file_handle("/a").unwrap();
        fs.take_snapshot(2);

        /* Test snapshot 0 */
        assert!(fs.restore_snapshot(0));
        assert!(fs.directory(a_dir).is_err());
        assert!(fs.file(a_file).is_err());
        assert!(fs.get_directory_handle("/a/").is_err());
        assert!(fs.get_file_handle("/a").is_err());

        /* Test snapshot 1 */
        assert!(fs.restore_snapshot(1));
        assert!(fs.directory(a_dir).is_ok());
        assert!(fs.file(a_file).is_err());
        assert!(fs.get_directory_handle("/a/").is_ok());
        assert!(fs.get_file_handle("/a").is_err());

        /* Test snapshot 2 */
        assert!(fs.restore_snapshot(2));
        assert!(fs.directory(a_dir).is_err());
        assert!(fs.file(a_file).is_ok());
        assert!(fs.get_directory_handle("/a/").is_err());
        assert!(fs.get_file_handle("/a").is_ok());
    ...
    fn chroot_escape_1() {
        ...
        let root = fs.get_directory_handle("/").unwrap();
        fs.mkdir(root, "test", PERM_X).unwrap();
        fs.mkdir(root, "asdf", PERM_X).unwrap();

        let test = fs.get_directory_handle("/test").unwrap();
        fs.chdir(test).unwrap();

        let asdf = fs.get_directory_handle("../asdf").unwrap();

        fs.chroot(test).unwrap();

        assert!(fs.dir_is_outside_root(root).unwrap());
        assert!(!fs.dir_is_outside_root(test).unwrap());

        fs.chdir(asdf).unwrap();

        assert!(fs.dir_is_outside_root(fs.cwd()).unwrap());
    }
    ...
    fn chroot_escape_2() {
        ...
        let root = fs.get_directory_handle("/").unwrap();
        fs.mkdir(root, "test", PERM_X).unwrap();
        fs.touch(root, "asdf", 0).unwrap();

        let test = fs.get_directory_handle("/test").unwrap();
        fs.chdir(test).unwrap();

        let asdf = fs.get_file_handle("../asdf").unwrap();

        fs.chroot(test).unwrap();

        assert!(fs.get_file_handle("../asdf").is_err());
        ...
        assert!(fs.file_is_outside_root(asdf).unwrap());
    }
}
